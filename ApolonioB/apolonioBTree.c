...
* @date 06 MAI 2021
* @brief  ArvoreB Sr. Apolonioa
*/

/*
OBSERVAÇÃO:
Professor, aprendemos o que faltava no nosso trabalho neste último P aluno, porém tomamos como prioridade
a prova.
O QUE FALTAVA?
- criar um arquivo adequado para a árvore B, contendo (no->count) (chaves e rrn das chaves) (filhos das chaves)
- retornar por parâmetro o rrn para que fosse inserido no arquivo
*/
...
void DivideNo(int val, int *pval, int pos, struct NoArvoreB *No, struct NoArvoreB *filho, struct NoArvoreB **NovoNo) {
  ...
  *NovoNo = (struct NoArvoreB *)malloc(sizeof(struct NoArvoreB));
  j = mediano + 1;
  while (j <= MAX) { //Insira os elementos em ordem crescente no novo nó verificando se o nó esta cheio
    (*NovoNo)->val[j - mediano] = No->val[j]; //o novo nó na posição 1 recebe o valor do nó da posição mediano + 1, posteriormente a posição mediano será promovida
  ...
  } else {// Inserindo no vetor2 do split 1 to 2
...
int DefineValor(int val, int *pval, struct NoArvoreB *No, struct NoArvoreB **filho) {
  ...
  if (!No) {//arvore vazia
    *pval = val;
    *filho = NULL; //porque a arvore esta vazia
    return 1;
  }

  if (val < No->val[1]) {//val menor que o primeiro valor do nó analisado
    pos = 0;
  } else {
  ...
  if (DefineValor(val, pval, No->registro[pos], filho)) { //Procure o nó apropriado para inserção.
    ...
    } else { //Se o nó estiver cheio faremos split
...
void conteudo(FILE* entrada, arqdados aux, indp* prim, ind_sforte *forte, ind_sfraco *fraco, complemento *comp, FILE *arqprimario){
  ...
  while(fscanf(entrada, "%3d %25[^\n] %25[^\n] %20[^\n] %2[^\n]", &aux.id, aux.nome, aux.rua, aux.cidade, aux.estado) != EOF){
    ...
    prim[i].rrn = i;
    forte[i].rrn = i;
    Insere(aux.id);//passa o id para inserir
    i++;
